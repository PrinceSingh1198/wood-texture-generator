...
# Define the modified Generator class to match the state_dict
...
state_dict = torch.load('/Users/princesingh/Desktop/3/gen.pth', map_location=torch.device('cpu'))

# Create an instance of the modified Generator
model = Generator()

# Load the state_dict with modification
model_dict = model.state_dict()
state_dict = {k: v for k, v in state_dict['generator'].items() if k in model_dict}
...
def display_form():
    if request.method == 'POST':
        ...
        plt.axis("off")
        plt.title("Generated Image (nz={})".format(nz))
